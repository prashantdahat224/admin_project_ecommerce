...
export default function ManageImageMainCategory() {
  ...
  const fetchCategories = async () => {
    ...
    if (!error && data?.main_category_image) {
      const { data: urlData } = supabase.storage
        .from("category-images")
        .getPublicUrl(data.main_category_image);
      setImagePath(data.main_category_image);   // file path (truth)
  ...
  const handleUpload = async () => {
    ...
    const filePath = `main_categories/${productId}/${Date.now()}.${fileExt}`;
    ...
    const { error: dbError } = await supabase
      .from("main_categories")
      .update({ main_category_image: filePath })
      .eq("id", productId);
  ...
  const handleDelete = async () => {
    ...
    const { error: dbError } = await supabase
      .from("main_categories")
      .update({ main_category_image: null })
      .eq("id", productId);
    ...
    <div>
      ...
      <div className="sticky top-0 bg-white z-50">
        <div className="flex items-center gap-2 ml-4">
          ...
          <h1 className="text-lg font-semibold"> Main Category image edit </h1>
          <button
      ...
      <div className="p-6 max-w-md mx-auto">
        <h1 className="text-xl font-bold mb-4">Main Category</h1>
