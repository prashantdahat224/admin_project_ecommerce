 ...
 // const dispatch = useDispatch();
  ...
  const addCategory = async () => {
   ...
   if(categories.length > 0 && categories.some(cat => cat.name.toLowerCase() === name.toLowerCase())){
    ...
    const { data, error } = await supabase.from("brand").insert([{ name: name_lower }]);
    if (error) {
      setLoading(false);
    } else {
    ...
    <div>
      ...
      <div className="sticky top-0 bg-white z-50">
        <div className="flex items-center gap-2 ml-4">
          ...
          <h1 className="text-lg font-semibold"> Product brands edit </h1>
            <button
      ...
      <div className="p-4 max-w-md mx-auto">
        ...
        <div className=" border-2 border-gray-500 p-2 mt-2">
          <p className="font-semibold">{item_name || "no brand selected"}</p>
           <input
                   type="number"
        ...
        <div className="space-y-2 mt-5">
          <p className="font-semibold">Existing brands:</p>
          {categories.map((cat) => (
            <div
              key={cat.id}
