...
export default function KeywordCategoryPage() {
  ...
  const fetchCategories_t = async () => {
    const { data, error } = await supabase
      .from("products")
      //.select("*")
      .select("category_id,category_name,search_keywords,brand_name,brand_id")
      .eq("id", productId)
      .single();
    if (!error && data) {
  ...
  const handleUploadBrand = async () => {
    ...
    if (selectedBrand.trim() === "" && newBrand.trim() === "") {
      alert("please select or enter the new brand");
      return;
    }
    ...
    <div>
      ...
      <div className="sticky top-0 bg-white z-50">
        <div className="flex items-center gap-2 ml-1">
          ...
          <h1 className="text-base font-semibold">Cetegory,brand,Keywords </h1>
      ...
      <div className="p-6 max-w-lg mx-auto space-y-6">
        ...
        {console.log("editKeywordAndCategory")}
        ...
        <div>
          ...
          <div className="border border-gray-500 p-2 bg-gray-100 mt-2">
        ...
        <div className="flex flex-col gap-2 ">
          ...
          <button
            ...
            disabled={loading}
            className="border border-blue-500 text-blue-500 px-4 py-2 rounded mt-3"
          >
            Upload Category
          </button>
        ...
        <div>
          ...
          <div className="border border-gray-500 p-2 bg-gray-100 mt-2">
            <p className="text-lg font-semibold">current stauts </p>
            <p>existing brand of product: {existingData_now?.brand_name ?? "no brand exist"} </p>
            <p></p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 ">
          <div className="border border-gray-500 p-2 rounded">
            <p className="text-lg font-semibold">brand of product</p>
            <p className="text-sm text-gray-500">select the brand or create
              the new one by entering name of brand</p>
            <div>
              <select
                className="w-full border rounded p-2 mt-2"
                value={selectedBrand}
              ...
              >
                ...
                {brands.map((brand_item) => (
                  <option key={brand_item.id} value={brand_item.id}>
                    {brand_item.name}
                  </option>
                ))}
              </select>
